...
function createDefaultTree(): SkillNode[] {
  return [
    ...
    node('w1', 'Power Strike', '+3 attack per rank', 'warrior', 1, 3, null, { flatDamage: 3 }),
    node('w2', 'Brute Force', '+2 strength per rank', 'warrior', 2, 3, 'w1', { strength: 2 }),
    node('w3', 'Frenzy', '+8% attack speed per rank', 'warrior', 3, 2, 'w2', { attackSpeed: 0.08 }),
    ...
    node('s1', 'Quick Feet', '+5% move speed per rank', 'scout', 1, 3, null, { moveSpeed: 0.05 }),
    node('s2', 'Sharp Eyes', '+2% crit chance per rank', 'scout', 2, 3, 's1', { critChance: 0.02 }),
    node('s3', 'Agility Training', '+2 agility per rank', 'scout', 3, 2, 's2', { agility: 2 }),
