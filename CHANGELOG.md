...
- **Merged Dependabot #214, bumping `three` 0.185.1 → 0.186.0 with `@types/three` alongside it** — Both packages moved in the one PR, which confirms the `three` group and the `exclude-patterns` fix from 2026-09-10 are working as intended. All nine checks were green. The `three` bundle chunk grows 153.12 kB → 155.36 kB gzipped, unremarkable against the 5 MB budget.
- **Asked Dependabot to rebase #215, which #214 pushed into conflict** — The grouped dev-dependency PR (`@gltf-transform/core` 4.4.2 → 4.5.0, `@playwright/test` 1.62.1 → 1.63.0, `eslint` 10.9.1 → 10.10.0, `lint-staged` 17.4.1 → 17.5.1, `typescript-eslint` 8.69.0 → 8.70.0, `vite` 8.2.2 → 8.3.0) was green on all nine checks, but `@types/three` sits between `@playwright/test` and `@vitest/coverage-v8` in `devDependencies`, so #214 collided with both of its neighbouring edits. Confirmed by test-merging it locally against the new `main` rather than waiting on GitHub'"'"'s mergeability computation, which still reported `unknown`. Left to `@dependabot rebase` so the lockfile is regenerated by the tooling against the new base instead of being hand-stitched from both sides.
