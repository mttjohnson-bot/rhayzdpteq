...
Resolve the four open Dependabot PRs. Two were already mergeable; the other two were the two halves of a single major bump that could never pass CI apart, so the pair is applied here as one change and the grouping rule that split them is corrected.

- **Bumped `vitest` and `@vitest/coverage-v8` together, 4.1.11 → 5.0.1** — This is the change Dependabot #211 and #212 were each trying to make one half of. `@vitest/coverage-v8` declares an *exact* peer on the runner (`peerDependencies.vitest "5.0.0"`), not a range, so bumping either package alone leaves `npm ci` unable to resolve the tree. Both PRs failed at the install step with `ERESOLVE` before a single check ran, which is why Lint & Format, Type Check, and npm audit all went red on each while the downstream jobs reported `skipped`. Neither PR was fixable on its own terms and no number of re-runs would have changed that. Applied as one commit, the install resolves cleanly and both packages land on 5.0.1 — a patch ahead of the 5.0.0 the PRs pinned, since the new `^5.0.0` range admits it.
- **Verified the major against the full gate suite, not just the install** — `npm ci` from a clean tree, lint, Prettier, `tsc --noEmit`, a production build, all 496 unit tests, and `npm run test:coverage` to exercise the coverage reporter itself, which is the package doing the breaking change. No source or config file needed editing; vitest 5 is a drop-in for this project's usage. `npm audit` reports zero vulnerabilities.
- **The vitest 5 tree is 13 packages smaller** — 2 added, 15 removed. `@vitest/expect`, `runner`, `snapshot`, `utils`, and `pretty-format` are no longer separate installs, and the coverage reporter now vendors its istanbul dependencies under `@vitest/istanbul-lib-coverage` and `@vitest/istanbul-lib-report` instead of pulling the upstream `istanbul-*` chain plus `make-dir`, `html-escaper`, and `supports-color`.
- **Added a `vitest` group to `.github/dependabot.yml` so the pair can never split again** — Modelled on the existing `three` group and placed directly after it, above the dev/production groups that would otherwise claim these packages first. Unlike `dev-dependencies`, it sets no `update-types` filter, so it claims majors too. The file's closing note that majors stay ungrouped is what produced #211 and #212 in the first place; it now records the exception for exact-peer pairs, where splitting a bump yields two unmergeable PRs rather than two independently reviewable ones.
- **Left Dependabot #214 and #215 alone — both were already green and need no intervention** — #214 bumps `three` 0.185.1 → 0.186.0 with `@types/three` moving with it in the same PR, confirming the `three` group is working as intended. #215 is the grouped dev-dependency PR: `@gltf-transform/core` 4.4.2 → 4.5.0, `@playwright/test` 1.62.1 → 1.63.0, `eslint` 10.9.1 → 10.10.0, `lint-staged` 17.4.1 → 17.5.1, `typescript-eslint` 8.69.0 → 8.70.0, and `vite` 8.2.2 → 8.3.0. All nine checks pass on each, including functional E2E and visual regression.
...
- **The Audit Fix workflow now fails when it falls back to opening an issue** — Previously it reported `success` on that path, which made a run needing human action indistinguishable from a run where nothing needed doing. That ambiguity is why three consecutive weekly runs (Aug 3, 10, and 17) correctly detected and pushed a fix that nobody acted on. The job now writes a run summary explaining the situation and then calls `core.setFailed`, so the run goes red in the Actions list and triggers GitHub's scheduled-workflow failure notification. The tracking issue is still created or updated first, so nothing is lost. The behaviour unwinds itself: if Actions is later permitted to open pull requests, the fallback never runs and the job stays green — and if that permission is revoked again, the signal comes back automatically.
- **Merged the two blocked Dependabot PRs** — `@playwright/test` 1.62.0 → 1.62.1, `eslint` 10.8.0 → 10.8.1, `jsdom` 30.0.0 → 30.0.1, `lint-staged` 17.2.0 → 17.3.0, `typescript-eslint` 8.65.0 → 8.67.0, and `vite` 8.1.5 → 8.2.1 (#202). Verified as a combined tree alongside the audit fix before merging, not just individually: lint, format, type check, 496 unit tests, and a production build. The `three` chunk is 153 kB gzipped, unchanged against the 5 MB budget.
...
- **Bumped `three` from 0.184.0 to 0.185.1 and `@types/three` from 0.184.1 to 0.185.1 (#195)** — Grouped update, so the library and its type definitions moved together. The rendering-sensitive gates all stayed green: type check, 496 unit tests, functional E2E, and visual regression. The `three` bundle chunk is 153 kB gzipped, comfortably inside the 5 MB budget.
- **Bumped `jsdom` from 29.1.1 to 30.0.0 (#196)** — Major version, dev-only (the unit-test DOM environment; nothing ships to players). The sole breaking change is a raised Node floor of `^22.22.2 || ^24.15.0 || >=26.0.0`. CI runs `node-version: 22`, which resolves to the latest 22.x and satisfies it. Also pulls `undici` 7.28.0 → 8.9.0 and `whatwg-url` 16 → 17 transitively, and adds `CSS.escape()`/`CSS.supports()` plus `getComputedStyle()` pixel conversion.
...
- **Held back `typescript` 6.0.3 → 7.0.2 (#192) and added a Dependabot ignore rule for the 7.x line** — Blocked upstream: `typescript-eslint` (including 8.65.0) declares a peer range of `>=4.8.4 <6.1.0`, so TypeScript 7 fails `npm ci` with `ERESOLVE`. The ignore is scoped to `7.x` in `.github/dependabot.yml` rather than applied via a `@dependabot` comment, so it lives in version control where it can be seen and removed, instead of in Dependabot's hidden per-repository state. TypeScript 6.x minor and patch updates still come through, and TypeScript 8 will be offered when it lands. Remove the entry once `typescript-eslint` widens its peer range.
...
- **Hardened the Audit Fix workflow against blocked pull-request creation** — The repository has "Allow GitHub Actions to create and approve pull requests" disabled, which no workflow `permissions:` block can override, so `pulls.create` returns 403. The workflow now catches that specific refusal and opens (or refreshes) a single tracking issue instead, carrying the same version table, audit state, and quality-gate results plus a one-click compare link. The fix branch is pushed either way, so the verified change is never lost. Any other API error still fails the run loudly.
